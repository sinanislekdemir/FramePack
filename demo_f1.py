...
from transformers import (CLIPTextModel, CLIPTokenizer, LlamaModel,
                          LlamaTokenizerFast, SiglipImageProcessor,
                          SiglipVisionModel)
...
from diffusers_helper.memory import (
    DynamicSwapInstaller, cpu, fake_diffusers_current_device,
    get_cuda_free_memory_gb, gpu, load_model_as_complete,
    move_model_to_device_with_memory_preservation,
    offload_model_from_device_for_memory_preservation, unload_complete_models)
from diffusers_helper.models.hunyuan_video_packed import \
    HunyuanVideoTransformer3DModelPacked
from diffusers_helper.pipelines.k_diffusion_hunyuan import sample_hunyuan
from diffusers_helper.utils import (crop_or_pad_yield_mask, generate_timestamp,
                                    resize_and_center_crop, save_bcthw_as_mp4,
                                    soft_append_bcthw)

parser = argparse.ArgumentParser(description="FramePack-F1 CLI")
parser.add_argument(
    "image", type=str, help="Path to input image or directory of images"
)
parser.add_argument("--prompt", type=str, required=True, help="Text prompt")
parser.add_argument(
    "--seed", type=int, default=31337, help="Random seed (default: 31337)"
)
parser.add_argument(
    "--seconds", type=float, default=5.0, help="Video length in seconds (default: 5)"
)
parser.add_argument(
    "--no-teacache", action="store_true", help="Disable TeaCache (default: enabled)"
)
args = parser.parse_args()

# Fixed defaults matching demo_gradio_f1.py
n_prompt = ""
...
def generate(
    ...
    height, width = find_nearest_bucket(*input_image.shape[:2], resolution=640)
    ...
    for section_index in range(total_latent_sections):
        ...
        def callback(d):
            ...
            frames_so_far = int(max(0, total_generated_latent_frames * 4 - 3))
            secs_so_far = max(0, (total_generated_latent_frames * 4 - 3) / 30)
...
print(f"Length: {args.seconds}s")
print(f"TeaCache: {use_teacache}")

try:
    input_paths = resolve_input_paths(args.image)
    print(f"Input files: {len(input_paths)}")

    for index, input_path in enumerate(input_paths, start=1):
        print(f"\n[{index}/{len(input_paths)}] Processing: {input_path}")
