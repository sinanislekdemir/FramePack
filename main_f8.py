...
# ---------------------------------------------------------------------------
# Lazy transformer loading (supports FP8 and standard models)
# ---------------------------------------------------------------------------
...
def load_fp8_transformer(model_info):
    ...
    weight_repo = model_info["weight_repo"]
    weight_file = model_info["weight_file"]
    config_repo = model_info["config_repo"]

    print(f"Loading FP8 model (config from {config_repo}, weights from {weight_repo}) ...")
    fp8_path = hf_hub_download(weight_repo, weight_file)

    config = HunyuanVideoTransformer3DModelPacked.load_config(config_repo)
    model = HunyuanVideoTransformer3DModelPacked.from_config(config).cpu().to(torch.bfloat16)

    with sf.safe_open(fp8_path, framework="pt") as f:
        keys = list(f.keys())
    print(f"  Memory-mapped loading {len(keys)} tensors (FP8 -> bf16) ...")
    with sf.safe_open(fp8_path, framework="pt") as f:
        for k in keys:
